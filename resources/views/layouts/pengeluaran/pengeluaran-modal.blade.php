<x-modal title="Tambah Pengeluaran" action="{{ $action }}" method="POST">
    ...
    <div class="mb-3">
        <label for="kategori_pengeluaran_id" class="form-label">Kategori <span class="text-danger">*</span></label>
        <select class="form-select" name="kategori_pengeluaran_id" id="kategori_pengeluaran_id" required>
            <option disabled selected class="text-muted">Pilih kategori pengeluaran</option>
            @foreach($kategoriPengeluarans as $kategoriPengeluaran)
                <option value="{{$kategoriPengeluaran->id}}" @if ($data->kategori_pengeluaran_id == $kategoriPengeluaran->id) selected @endif>{{$kategoriPengeluaran->name}}</option>
            @endforeach
        </select>
    ...
    <div class="mb-3">
        <label for="jumlah" class="form-label">Total Pengeluaran <span class="text-danger">*</span></label>
