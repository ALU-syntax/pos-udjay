...
## 6. Reward Confirmation Sync (Delta Sync)
...
Khusus endpoint claim sync (section 4–6).
...
1. Panggil ketiga endpoint catalog (section 1–3) dan ketiga endpoint claim sync (section 4–6) saat aplikasi connect / sebelum masuk mode offline.
...
> ⚠️ **WAJIB DIBACA AGENT MOBILE:** reward level punya **dua fungsi** di mobile — mencegah double claim **dan** menampilkan riwayat. Gunakan `reward-confirmations/sync` (section 6) untuk keduanya.
...
- `reward_memberships_id` didapat dari data reward level customer (endpoint `customers/{id}` atau `reward-memberships`).
