...
class ExpClaimSyncTest extends TestCase
{
    ...
    private function makeCustomer(array $attrs = []): Customer
    {
        return Customer::create(array_merge([
            'name' => 'Exp Claim Test ' . Str::random(6),
    ...
    private function makeClaim(array $attrs = []): ExpRewardClaims
    {
        $claim = ExpRewardClaims::create($attrs);

        // Bypass Eloquent agar updated_at tidak ditimpa menjadi waktu sekarang
        DB::table('exp_reward_claims')
            ->where('id', $claim->id)
    ...
    {
        $this->getJson('/api/v1/customers/exp-claims/sync')->assertStatus(401);
    }
    ...
    public function test_sync_returns_data_with_names_and_metadata()
    {
        ...
        $response = $this->actingAs($user, 'sanctum')
            ->getJson('/api/v1/customers/exp-claims/sync?updated_since=' . self::SINCE_BOUNDARY . '&limit=1000');
    ...
    public function test_sync_only_sends_rows_updated_after_since()
    {
        ...
        $oldClaim = ExpRewardClaims::create([
            'customer_id' => $customer->id,
            'outlet_id'   => $outlet->id,
            'product_id'  => $product->id,
            'exp'         => 5000,
    ...
    public function test_sync_paginates_with_cursor()
    {
        ...
        $first = $this->actingAs($user, 'sanctum')
            ->getJson('/api/v1/customers/exp-claims/sync?updated_since=' . self::SINCE_BOUNDARY . '&limit=1');
        ...
        $second = $this->actingAs($user, 'sanctum')
            ->getJson('/api/v1/customers/exp-claims/sync?cursor=' . urlencode($cursor) . '&limit=1000');
    ...
    public function test_sync_rejects_invalid_cursor()
    {
        ...
        $this->actingAs($user, 'sanctum')
            ->getJson('/api/v1/customers/exp-claims/sync?cursor=not-a-valid-cursor')
            ->assertStatus(422)
    ...
    public function test_sync_rejects_invalid_updated_since()
    {
        ...
        $this->actingAs($user, 'sanctum')
            ->getJson('/api/v1/customers/exp-claims/sync?updated_since=kemarin')
            ->assertStatus(422)
            ->assertJson(['status' => 'error']);
    }
}
